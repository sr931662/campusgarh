...
const CollegeSearchInput = ({ selected, onChange }) => {
  ...
  const search = (q) => {
    ...
    timer.current = setTimeout(async () => {
      try {
        ...
        setResults(res.data?.data?.colleges || []);
        setOpen(true);
      } catch { setResults([]); }
    }, 300);
  };
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: selected.length ? 8 : 0 }}>
        {selected.map(s => (
          <span key={s.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: '#eff6ff', color: '#1d4ed8', padding: '3px 10px', borderRadius: 20, fontSize: '0.78rem', fontWeight: 600 }}>
            {s.name}
            <button type="button" onClick={() => remove(s.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#1d4ed8', fontWeight: 700, padding: 0, lineHeight: 1 }}>×</button>
          </span>
        ))}
      </div>
      <input
        ...
        <ul style={{ position: 'absolute', zIndex: 50, width: '100%', background: '#fff', border: '1px solid var(--border, #E8E3DB)', borderRadius: 8, marginTop: 4, padding: 0, listStyle: 'none', boxShadow: '0 4px 16px rgba(0,0,0,0.10)', maxHeight: 220, overflowY: 'auto' }}>
          {results.map(c => (
            <li key={c._id} onMouseDown={() => add(c)}
              ...
              {c.name}
              {c.contact?.state && <span style={{ color: 'var(--muted)', fontSize: '0.72rem', marginLeft: 6 }}>{c.contact.state}</span>}
            </li>
          ))}
        </ul>
      )}
