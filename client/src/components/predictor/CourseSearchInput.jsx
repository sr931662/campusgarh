...
const CourseSearchInput = ({ selected, onChange, maxSelect = 10 }) => {
          ...
          {selected.map(s => (
            <span key={s.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: '#f0fdf4', color: '#166534', padding: '3px 10px', borderRadius: 20, fontSize: '0.78rem', fontWeight: 600 }}>
              ...
              <button type="button" onPointerDown={e => { e.preventDefault(); remove(s.id); }}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#166534', fontWeight: 700, padding: 0, lineHeight: 1, fontSize: '1rem' }}>×</button>
            </span>
          ))}
        </div>
      )}
        ...
        <ul
          ...
          {results.map(c => (
            <li key={c._id}
              ...
              <span>{c.name}</span>
              {c.category && <span style={{ color: 'var(--muted)', fontSize: '0.72rem' }}>{c.category}</span>}
            </li>
          ))}
        </ul>
      )}
